...
var init = {
  ...
  getTableName: function (type) {
    ...
    for (var i = 0; i < table.length; i++) {
      if (type == table[i].type) return table[i].name;
    }
  },
  time: [
    {
  ...
  getTimeName: function (type) {
    ...
    for (var i = 0; i < time.length; i++) {
      if (type == time[i].type) return time[i].name;
    }
  },
  loadData: function () {
    $.ajax({
      ...
      success: function (data) {
        if (data.code == "0") {
          ...
          if (queued.callNumber == "0") {
            ...
            $("#wait").html(
              "当前叫号：" +
                init.getTableName(queued.tableType) +
                queued.callNumber +
                "号"
            );
          }
    ...
    $("#dinging").click(function () {
      ...
      if ("1" == sessionStorage.getItem("hasDining")) {
        ...
        window.location.href =
          "reservation/dishes.html?storeId=" + storeId + "&orderId=" + orderId;
      }
    });
  },
};
